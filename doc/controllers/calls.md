...
```ts
const callsController = new CallsController(client);
```
...
try {
  const response = await callsController.callsAdd(
    token,
    externalUniqueId,
    joinUrl
  );
...
try {
  const response = await callsController.callsEnd(
    token,
    id
  );
...
try {
  const response = await callsController.callsInfo(
    token,
    id
  );
...
try {
  const response = await callsController.callsUpdate(
    token,
    id
  );
