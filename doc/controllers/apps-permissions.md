...
```ts
const appsPermissionsController = new AppsPermissionsController(client);
```
...
```ts
try {
  const response = await appsPermissionsController.appsPermissionsInfo();
...
try {
  const response = await appsPermissionsController.appsPermissionsRequest(
    token,
    scopes,
    triggerId
  );
