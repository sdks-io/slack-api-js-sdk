...
```ts
const usersProfileController = new UsersProfileController(client);
```
...
try {
  const response = await usersProfileController.usersProfileGet(token);
...
try {
  const response = await usersProfileController.usersProfileSet(token);
