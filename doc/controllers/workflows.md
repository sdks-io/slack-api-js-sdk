...
```ts
const workflowsController = new WorkflowsController(client);
```
...
try {
  const response = await workflowsController.workflowsStepCompleted(
    token,
...
try {
  const response = await workflowsController.workflowsStepFailed(
    token,
    workflowStepExecuteId,
    error
  );
...
try {
  const response = await workflowsController.workflowsUpdateStep(
    token,
    workflowStepEditId
  );
