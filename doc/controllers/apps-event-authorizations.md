...
```ts
const appsEventAuthorizationsController = new AppsEventAuthorizationsController(client);
```
...
try {
  const response = await appsEventAuthorizationsController.appsEventAuthorizationsList(
    token,
    eventContext
  );
