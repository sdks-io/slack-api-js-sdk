...
```ts
const dndController = new DndController(client);
```
...
try {
  const response = await dndController.dndEndDnd(token);
...
try {
  const response = await dndController.dndEndSnooze(token);
...
```ts
try {
  const response = await dndController.dndInfo();
...
try {
  const response = await dndController.dndSetSnooze(
    token,
    numMinutes
  );
...
```ts
try {
  const response = await dndController.dndTeamInfo();
