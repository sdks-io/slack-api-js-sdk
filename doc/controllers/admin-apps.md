...
```ts
const adminAppsController = new AdminAppsController(client);
```
...
try {
  const response = await adminAppsController.adminAppsApprove(token);
...
try {
  const response = await adminAppsController.adminAppsRestrict(token);
