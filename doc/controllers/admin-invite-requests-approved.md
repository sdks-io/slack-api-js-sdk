...
```ts
const adminInviteRequestsApprovedController = new AdminInviteRequestsApprovedController(client);
```
...
try {
  const response = await adminInviteRequestsApprovedController.adminInviteRequestsApprovedList(token);
