...
```ts
const chatScheduledMessagesController = new ChatScheduledMessagesController(client);
```
...
```ts
try {
  const response = await chatScheduledMessagesController.chatScheduledMessagesList();
