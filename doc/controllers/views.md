...
```ts
const viewsController = new ViewsController(client);
```
...
try {
  const response = await viewsController.viewsOpen(
    token,
    triggerId,
    view
  );
...
try {
  const response = await viewsController.viewsPublish(
    token,
    userId,
    view
  );
...
try {
  const response = await viewsController.viewsPush(
    token,
    triggerId,
    view
  );
...
try {
  const response = await viewsController.viewsUpdate(token);
