...
```ts
const adminTeamsController = new AdminTeamsController(client);
```
...
try {
  const response = await adminTeamsController.adminTeamsCreate(
    token,
    teamDomain,
    teamName
  );
...
try {
  const response = await adminTeamsController.adminTeamsList(token);
