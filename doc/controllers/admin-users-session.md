...
```ts
const adminUsersSessionController = new AdminUsersSessionController(client);
```
...
try {
  const response = await adminUsersSessionController.adminUsersSessionInvalidate(
    token,
    teamId,
    sessionId
  );
...
try {
  const response = await adminUsersSessionController.adminUsersSessionReset(
    token,
    userId
  );
