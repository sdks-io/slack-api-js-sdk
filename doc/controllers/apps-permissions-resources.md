...
```ts
const appsPermissionsResourcesController = new AppsPermissionsResourcesController(client);
```
...
try {
  const response = await appsPermissionsResourcesController.appsPermissionsResourcesList(token);
