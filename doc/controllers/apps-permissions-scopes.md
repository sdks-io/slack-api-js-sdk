...
```ts
const appsPermissionsScopesController = new AppsPermissionsScopesController(client);
```
...
try {
  const response = await appsPermissionsScopesController.appsPermissionsScopesList(token);
