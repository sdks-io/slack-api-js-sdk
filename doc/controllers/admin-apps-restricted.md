...
```ts
const adminAppsRestrictedController = new AdminAppsRestrictedController(client);
```
...
try {
  const response = await adminAppsRestrictedController.adminAppsRestrictedList(token);
