...
```ts
const adminInviteRequestsController = new AdminInviteRequestsController(client);
```
...
try {
  const response = await adminInviteRequestsController.adminInviteRequestsApprove(
    token,
    inviteRequestId
  );
...
try {
  const response = await adminInviteRequestsController.adminInviteRequestsDeny(
    token,
    inviteRequestId
  );
...
try {
  const response = await adminInviteRequestsController.adminInviteRequestsList(token);
