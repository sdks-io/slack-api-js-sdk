...
```ts
const starsController = new StarsController(client);
```
...
try {
  const response = await starsController.starsAdd(token);
...
```ts
try {
  const response = await starsController.starsList();
...
try {
  const response = await starsController.starsRemove(token);
