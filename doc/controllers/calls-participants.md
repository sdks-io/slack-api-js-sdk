...
```ts
const callsParticipantsController = new CallsParticipantsController(client);
```
...
try {
  const response = await callsParticipantsController.callsParticipantsAdd(
    token,
    id,
    users
  );
...
try {
  const response = await callsParticipantsController.callsParticipantsRemove(
    token,
    id,
    users
  );
