...
```ts
const usergroupsUsersController = new UsergroupsUsersController(client);
```
...
try {
  const response = await usergroupsUsersController.usergroupsUsersList(
    token,
    usergroup
  );
...
try {
  const response = await usergroupsUsersController.usergroupsUsersUpdate(
    token,
    usergroup,
    users
  );
