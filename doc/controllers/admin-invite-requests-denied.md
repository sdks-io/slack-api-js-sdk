...
```ts
const adminInviteRequestsDeniedController = new AdminInviteRequestsDeniedController(client);
```
...
try {
  const response = await adminInviteRequestsDeniedController.adminInviteRequestsDeniedList(token);
