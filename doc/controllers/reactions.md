...
```ts
const reactionsController = new ReactionsController(client);
```
...
try {
  const response = await reactionsController.reactionsAdd(
    channel,
    name,
    timestamp,
    token
...
try {
  const response = await reactionsController.reactionsGet(token);
...
try {
  const response = await reactionsController.reactionsList(token);
...
try {
  const response = await reactionsController.reactionsRemove(
    token,
    name
  );
