...
```ts
const appsPermissionsUsersController = new AppsPermissionsUsersController(client);
```
...
try {
  const response = await appsPermissionsUsersController.appsPermissionsUsersList(token);
...
try {
  const response = await appsPermissionsUsersController.appsPermissionsUsersRequest(
    token,
    scopes,
    triggerId,
    user
